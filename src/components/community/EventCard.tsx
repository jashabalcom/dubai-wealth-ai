...
export function EventCard({ event, onRegister, onUnregister, isRegistering }: EventCardProps) {
  ...
  const hasRecording = event.recording_url && event.recording_visible;
  const canAccessRecording = hasRecording && (
    event.recording_access === 'all_members' 
      ? (tier === 'investor' || tier === 'elite')
      : tier === 'elite'
  );
    ...
    <>
              ...
              hasRecording ? (
                canAccessRecording ? (
                  ...
                  >
                    ...
                    {event.recording_access === 'elite_only' ? 'Elite Only' : 'Upgrade to Watch'}
                  </Button>
                )
              ) : (
      ...
      <Dialog open={showRecordingModal} onOpenChange={setShowRecordingModal}>
        <DialogContent className="max-w-4xl">
          ...
          <div className="mt-4">
            <VideoPlayer url={event.recording_url} title={event.title} />
          </div>
          {event.description && (
